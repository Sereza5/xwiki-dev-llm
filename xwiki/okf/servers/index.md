...
| [ge.xwiki.org](https://ge.xwiki.org) | Develocity — stores CI build scans and provides build caching for CI and local builds. | **MCP: `develocity`** (this plugin) — build failures with stack traces, test outcomes and flaky-test history, build timings, cache effectiveness, build-to-build diffs. Remote (streamable HTTP) at `https://ge.xwiki.org/mcp`; needs `DEVELOCITY_MCP_ACCESS_KEY`, which is validated on *every* request including tool listing, so a missing key means the server is absent, not read-only. Build scans are also browsable at `https://ge.xwiki.org/scans`. |
